...
NUs = np.zeros(10)
for i in range (10):
    current_nu = i*0.05+0.05
    NUs[i] = current_nu
    #Training phase
    clf = svm.NuSVC(nu = current_nu, kernel = 'rbf', degree = 2, probability = True)
    clf.fit(fvtrain, labtrain) 
    #Validation phase to find optimal nu
    prediction = clf.predict_proba(fvvalid)
    AUCs[i] = roc_auc_score(labvalid, prediction[:,-1])

#Testing phase
index = np.argmax(AUCs)
optimal_nu = NUs[index]
clf = svm.NuSVC(nu = optimal_nu, kernel = 'rbf', degree = 2, probability = True)  
clf.fit(fvtrain, labtrain) 

#Evaluation of the AUC on the test set
test_pred = clf.predict_proba(fvtest)
